...
if st.session_state["alteracao_estoque_processando"] is not None:
    ...
    try:
        itens_alteracao = alteracao.get("itens") or []

        if not itens_alteracao:
            itens_alteracao = [{
                "codigo_produto": alteracao["codigo_produto"],
                "produto_nome": alteracao["produto_nome"],
                "estoque_atual": alteracao["estoque_atual"],
        ...
        with st.spinner("Alterando estoque. Aguarde..."):
            for item_alteracao in itens_alteracao:
                estoque_atual = int(float(item_alteracao["estoque_atual"]))
                nova_quantidade = int(float(item_alteracao["nova_quantidade"]))
        ...
        if quantidade_registrada > 1:
            st.session_state["mensagem_sucesso"] = f"{quantidade_registrada} alterações de estoque registradas com sucesso."
        else:
...
if st.session_state["confirmar_alteracao_estoque"] is not None:
    ...
    for item_alteracao in itens_alteracao:
        ...
        if diferenca > 0:
            ajuste = f"Entrada de {formatar_numero_exibicao(abs(diferenca))}"
        elif diferenca < 0:
            ajuste = f"Saída de {formatar_numero_exibicao(abs(diferenca))}"
        else:
            ajuste = "Sem alteração"

        linhas_confirmacao.append({
            "Produto": item_alteracao["produto_nome"],
            "Estoque atual": formatar_numero_exibicao(estoque_atual),
            "Nova quantidade": formatar_numero_exibicao(nova_quantidade),
            "Ajuste": ajuste
        })

    st.dataframe(
        pd.DataFrame(linhas_confirmacao),
...
try:
    ...
    if aba_atual == "Consulta de Estoque":
        ...
        if produtos_ativos.empty:
            ...
            produtos_ativos["estoque_atual_int"] = (
                pd.to_numeric(produtos_ativos["estoque_atual"], errors="coerce")
                .fillna(0)
                .round(0)
                .astype(int)
            )
            produtos_ativos["unidade_exibir"] = produtos_ativos["unidade"].fillna("").astype(str)
            produtos_ativos["produto_opcao_alteracao"] = produtos_ativos.apply(
                lambda row: (
                    f"{row['codigo']} - {row['nome']} | "
                    f"Estoque atual: {formatar_numero_exibicao(row['estoque_atual_int'])}"
                    f"{(' ' + row['unidade_exibir']) if row['unidade_exibir'] else ''}"
                ),
                axis=1
            )

            opcao_placeholder = "Selecione um produto"
            opcoes_produtos_alteracao = [opcao_placeholder] + produtos_ativos["produto_opcao_alteracao"].dropna().astype(str).tolist()
            mapa_opcoes_alteracao = {}

            for _, produto_linha in produtos_ativos.iterrows():
                opcao = str(produto_linha["produto_opcao_alteracao"])
                mapa_opcoes_alteracao[opcao] = {
                    "codigo_produto": str(produto_linha["codigo"]),
                    "produto_nome": str(produto_linha["nome"]),
                    "estoque_atual": int(produto_linha["estoque_atual_int"]),
                    "unidade": str(produto_linha.get("unidade", "") or ""),
                    "status": str(produto_linha.get("status", "") or "").upper()
                }

            with st.form(f"form_alteracao_estoque_{st.session_state['reset_alteracao_estoque']}"):
                st.caption(
                    "Informe o motivo e ajuste a nova quantidade em estoque. "
                    "O estoque atual aparece no próprio campo do produto. "
                    "Use o botão de + da tabela para alterar vários produtos."
                )
                ...
                df_alteracao_editor = st.data_editor(
                    pd.DataFrame([{"Produto": opcao_placeholder, "Nova quantidade": None}]),
                    use_container_width=True,
                    hide_index=True,
                    num_rows="dynamic",
                    column_config={
            ...
            if botao_alterar_estoque:
                ...
                if df_alteracao_editor is not None and not df_alteracao_editor.empty:
                    for _, linha_alteracao in df_alteracao_editor.iterrows():
                        ...
                        nova_quantidade = linha_alteracao.get("Nova quantidade", None)

                        produto_informado = bool(produto_alteracao and produto_alteracao != opcao_placeholder)
                        quantidade_informada = nova_quantidade is not None and not pd.isna(nova_quantidade)

                        if not produto_informado and not quantidade_informada:
                            continue

                        if not produto_informado:
                            exibir_alerta_temporario("Selecione o produto em todas as linhas preenchidas.", tipo="error")
                            st.stop()

                        if not quantidade_informada:
                            exibir_alerta_temporario("Informe a nova quantidade em todas as linhas preenchidas.", tipo="error")
                            st.stop()

                        try:
                            nova_quantidade = int(float(nova_quantidade))
                        except Exception:
                            exibir_alerta_temporario("A nova quantidade deve ser um número inteiro.", tipo="error")
                            st.stop()
                        ...
                        dados_produto = mapa_opcoes_alteracao.get(produto_alteracao)
                        ...
                        if codigo_produto in codigos_informados:
                            exibir_alerta_temporario("O mesmo produto foi informado mais de uma vez.", tipo="error")
                            st.stop()

                        codigos_informados.add(codigo_produto)

                        estoque_atual = int(dados_produto["estoque_atual"])
                        ...
                        itens_alteracao.append({
                            "codigo_produto": codigo_produto,
                            "produto_nome": dados_produto["produto_nome"],
                            "estoque_atual": estoque_atual,
                            "nova_quantidade": nova_quantidade
                        })

                if not itens_alteracao:
                    exibir_alerta_temporario("Informe pelo menos um produto com nova quantidade diferente do estoque atual.", tipo="warning")
                    st.stop()
        ...
        with st.form(f"form_saida_produtos_{tipo_saida}_{tipo_monzi}_{st.session_state['reset_saida']}"):
            pedido_saida = st.text_input(
                ...
                placeholder="Ex: PED123" if tipo_saida != "OUTROS" else "Ex: PED123",
        ...
        historico["_data_ordenacao"] = pd.to_datetime(
            historico["criado_em"],
            format="%d/%m/%Y %H:%M:%S",
            errors="coerce"
        )
        historico["_id_ordenacao"] = pd.to_numeric(
            historico["id"].astype(str).str.extract(r"(\d+)")[0],
            errors="coerce"
        ).fillna(0)
